...
function update_calendar(){
  ...
  $.each(current_month.dates, function(i, day){
    var color = {2: "rgba(39, 174, 96,0.8)", 1: "rgba(39, 174, 96,0.2)", 0: "white"}
    $('[data-date-iso="' + day.date + '"]').css('background', color[day.valid])
    if(day.valid != 0){
      var tag = $('<h4></h4>')
        .addClass('volunteer')
        .append(day.volunteer_hours + " Hrs Volunteered")
...
function update_day(day){
  reset('hours')
  $.each(day.hours, function(i, hour){
    var color = {2: "rgba(39, 174, 96,0.8)", 1: "white"}

    $('#' + hour.hour).css('background', color[hour.valid])
    if(hour.valid != 0){
      var tag = $('<strong></strong>')
        .addClass('volunteer')
...
function reset(selector){
  $('.' + selector).css('background', 'rgba(149, 165, 166,0.4)')
  $('.volunteer').remove()
}
